      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <Link href="/admin/workspaces" className="block">
            <Card className="bg-slate-900 border-slate-800 p-4 hover:border-blue-600 transition-colors cursor-pointer">
              <h3 className="text-sm font-medium text-slate-300 mb-1">Workspaces</h3>
              <p className="text-xs text-slate-500">Manage all workspaces & subscriptions</p>
            </Card>
          </Link>
          <Link href="/admin/users" className="block">
            <Card className="bg-slate-900 border-slate-800 p-4 hover:border-blue-600 transition-colors cursor-pointer">
              <h3 className="text-sm font-medium text-slate-300 mb-1">Users</h3>
              <p className="text-xs text-slate-500">View all registered users</p>
            </Card>
          </Link>
          <Link href="/admin/licenses" className="block">
            <Card className="bg-slate-900 border-slate-800 p-4 hover:border-blue-600 transition-colors cursor-pointer">
              <h3 className="text-sm font-medium text-slate-300 mb-1">License Keys</h3>
              <p className="text-xs text-slate-500">Generate & manage license keys</p>
            </Card>
          </Link>
        </div>
        ...
        <Card className="bg-slate-900 border-slate-800 p-4">
          <h3 className="text-sm font-medium text-slate-300 mb-3">Recent Signups</h3>
              ...
              <tbody>
                {data?.recentSignups.map((u) => (
                  <tr key={u.id} className="border-b border-slate-800/50 text-slate-300">
                    ...
                    <td className="py-2 text-slate-400">{format(new Date(u.createdAt), "MMM d, yyyy")}</td>
                    <td className="py-2 text-slate-400">{u.lastLoginAt ? format(new Date(u.lastLoginAt), "MMM d, HH:mm") : "Never"}</td>
                  </tr>
                ))}
                {isLoading && (
