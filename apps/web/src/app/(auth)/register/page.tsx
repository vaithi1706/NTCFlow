...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!acceptTerms) { toast.error("You must accept the terms of service"); return; }
    setLoading(true);
    try {
      const result = await trpcVanilla.auth.register.mutate({ name, email, password });
      setAuth(result.user, result.accessToken, result.refreshToken, result.workspaceId ?? undefined);
      toast.success("Account created! Welcome to DKFlow.");
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md border-border">
        <CardHeader className="text-center space-y-2 pb-2">
          ...
          <p className="text-sm text-muted-foreground">Start managing projects with DKFlow</p>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name">Full Name</Label>
              <Input id="name" placeholder="Danish Khan" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
            </div>
            <div className="space-y-2">
