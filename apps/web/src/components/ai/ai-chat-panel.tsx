  ...
  return (
    <div className="flex justify-start">
      <div className="flex items-start gap-2 max-w-[85%]">
        ...
        <div className="bg-slate-800/80 border border-slate-700/50 rounded-xl rounded-bl-sm px-4 py-3">
    ...
    <ReactMarkdown
      components={{
        ...
        code: ({ className, children }) => {
          ...
          return <code className="bg-slate-900 px-1.5 py-0.5 rounded text-xs text-emerald-400 font-mono">{children}</code>;
        },
        pre: ({ children }) => <>{children}</>,
        blockquote: ({ children }) => <blockquote className="border-l-2 border-violet-500 pl-3 my-2 text-slate-400 italic">{children}</blockquote>,
      }}
    >
      {content}
    </ReactMarkdown>
    ...
    <>
      ...
      {open && (
        <>
          ...
          {/* Panel */}
          <div className="fixed right-0 top-0 h-full w-[520px] max-w-full bg-slate-950 border-l border-slate-800 z-50 flex shadow-2xl shadow-black/50 animate-in slide-in-from-right duration-300">

            {/* Sidebar */}
            {sidebarOpen && (
              <div className="w-[180px] bg-slate-900 border-r border-slate-800 flex flex-col flex-shrink-0">
                <div className="p-2">
                  <button
                    onClick={handleNewChat}
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-slate-300 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors"
                  >
                ...
                <div className="flex-1 overflow-y-auto px-1.5 pb-2 scrollbar-thin scrollbar-thumb-slate-700">
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="flex items-center justify-between px-3 py-2.5 border-b border-slate-800 bg-slate-950">
                    ...
                    {editingTitle && activeSessionId ? (
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          className="bg-slate-800 border border-slate-600 rounded px-2 py-0.5 text-xs text-white w-full focus:outline-none focus:border-violet-500"
                          autoFocus
                        />
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-white flex-shrink-0" onClick={handleClose}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-slate-800">
                {messages.length === 0 && !historyQuery.isLoading && (
                  <div className="flex flex-col items-center justify-center h-full text-center space-y-6 px-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-white mb-1">Hi! I&apos;m your AI Project Manager</h3>
                      <p className="text-sm text-slate-400">I can help you manage tasks, analyze sprints, and more</p>
                    </div>
                    <div className="grid grid-cols-2 gap-2 w-full max-w-[320px]">
                      {SUGGESTIONS.map(s => (
                        <button
                          ...
                          className="text-left text-xs px-3 py-3 rounded-xl border border-slate-800 hover:border-violet-500/30 hover:bg-violet-500/5 transition-all text-slate-400 hover:text-slate-200 group"
                        >
                  ...
                  <div key={msg.id || i} className={cn("flex", msg.role === "user" ? "justify-end" : "justify-start")} style={{ animation: "fadeIn 0.3s ease-out" }}>
                    {msg.role === "assistant" ? (
                      <div className="flex items-start gap-2 max-w-[88%]">
                        ...
                        <div>
                          <div className="bg-slate-800/80 border border-slate-700/50 rounded-xl rounded-bl-sm px-3.5 py-2.5 text-sm text-slate-100">
                ...
                <div className="px-3 pt-2 flex gap-1.5 flex-wrap">
                  {QUICK_ACTIONS.map(a => (
                    <button
                      ...
                      className="flex items-center gap-1 text-[10px] px-2 py-1 rounded-full border border-slate-700 text-slate-400 hover:text-violet-300 hover:border-violet-500/30 transition-colors disabled:opacity-50"
                    >
              ...
              <div className="p-3 border-t border-slate-800">
                <div className="flex gap-2 items-end">
                  <textarea
                    ...
                    rows={1}
                    className="flex-1 bg-slate-900 border border-slate-700 focus:border-violet-500/50 rounded-xl px-3.5 py-2.5 text-sm text-white placeholder-slate-500 resize-none focus:outline-none focus:ring-1 focus:ring-violet-500/25 transition-colors"
                    disabled={chatMutation.isPending}
                    style={{ maxHeight: 120 }}
                  />
                  <Button
                ...
                <p className="text-[10px] text-slate-600 mt-1.5 text-center">Shift+Enter for new line</p>
              </div>
            </div>
          </div>
        </>
      ...
      <style jsx global>{`
        ...
        .scrollbar-thumb-slate-700::-webkit-scrollbar-thumb { background: rgb(51 65 85); }
        .scrollbar-thumb-slate-800::-webkit-scrollbar-thumb { background: rgb(30 41 59); }
      `}</style>
    </>
  );
}
